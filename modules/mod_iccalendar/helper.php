...
//		$days=$this->getDays($this->date_start, $this->format);
		...
		foreach ($res as $r) {
			...
			$StDate = date('Y-m-d H:i', $this->mkt($r->startdate));
			$EnDate = date('Y-m-d H:i', $this->mkt($r->enddate));
...
//			$list[$a]['plus'] = "$stamp->events[1]['cat_color']";
	...
	function getNav($date_start, $modid)
	{
		...
		$backY='<div class="backicY icagendabtn_'.$modid.'" href="'.JRoute::_($Aurl.'&date='.$backYYear.'-'.$month.'-'.$day).'">&#9668;&#9668;</div>';
		$back='<div class="backic icagendabtn_'.$modid.'" href="'.JRoute::_($Aurl.'&date='.$backYear.'-'.$backMonth.'-'.$day).'">&#9668;</div>';

		$next='<div class="nextic icagendabtn_'.$modid.'" href="'.JRoute::_($Aurl.'&date='.$nextYear.'-'.$nextMonth.'-'.$day).'">&#9658;</div>';
		$nextY='<div class="nexticY icagendabtn_'.$modid.'" href="'.JRoute::_($Aurl.'&date='.$nextYYear.'-'.$month.'-'.$day).'">&#9658;&#9658;</div>';
