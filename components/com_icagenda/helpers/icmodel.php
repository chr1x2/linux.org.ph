...
//		$sdate    = strtotime("$start +1 day");
	...
	protected function getDBitems()
	{
		...
		// Selectable items
		$query->select('next AS tNext, dates AS tDates, period AS tPeriod, startdate AS tStartdate, enddate AS tEnddate, weekdays AS tWeekdays, id AS tId, state AS tState, access AS tAccess');
...
//			if (isset($tPeriod)) {
//				$period=unserialize($tPeriod);
//			} else {
//				$period=unserialize($tDates);
//			}
//print_r($dates);
...
//			if (($daysdtest == '0000-00-00 00:00:00') AND ($period != NULL)) {
//				$DatesList=$period;
//				$AllDates=$DatesList;
//			}
//			if (($daysdtest != '0000-00-00 00:00:00') AND ($period != NULL)) {
//				$DatesList=array_merge($dates, $period);
//				$AllDates=$DatesList;
//			}
//			if (($daysdtest != '0000-00-00 00:00:00') AND ($period == NULL)) {
//				$DatesList=$dates;
...
//	}
	...
	protected function header ()
	{
		...
		// Adding Filter per Category in Navigation - Bug Fixed 1.2.6 released
		$selcat = '';
...
//		$user = JFactory::getUser();
//		$userLevels = $user->getAuthorisedViewLevels();
//		if(isset($userLevels)) $level=max ($user->getAuthorisedViewLevels());
		...
		if ($v==0){
			...
			$where= "(e.state = 1) AND (e.approval <> 1) AND (e.next != '$nodate') AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}
		...
		if ($v==1){
			...
			$where= "(e.next >= (CURDATE())) AND (e.next != '$nodate') AND (e.state = 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}
		...
		if ($v==2){
			...
			$where= "(e.next < (CURDATE())) AND (e.next != '$nodate') AND (e.state = 1) AND (e.approval <> 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}

		if ($v==3){
			$where= "(e.next >= (NOW())) AND (e.next != '$nodate') AND (e.state = 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}

		if ($v==4){
			$where= "(e.next >= (CURDATE())) AND (e.next < (CURDATE() + INTERVAL 1 DAY)) AND (e.next != '$nodate') AND (e.state = 1) AND (e.approval <> 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}
		...
		if ($headerList == 1) $header='<div><h2> '.$timetitle.' <br/></h2> '.$report.' '.$report2.'';
		if ($headerList == 2) $header='<div><h2> '.$timetitle.' </h2><br/>';
...
//		if(isset($userLevels)) $level=max ($user->getAuthorisedViewLevels());
		...
		if ($v==0){
			$where= "(e.state = 1) AND (e.approval <> 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}

		if ($v==1){
			$where= "(e.next >= (CURDATE())) AND (e.state = 1) AND (e.approval <> 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}
		...
		if ($v==3){
			$where= "(e.next >= (NOW())) AND (e.state = 1) AND (e.approval <> 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}

		if ($v==4){
			$where= "(e.next >= (CURDATE())) AND (e.next < (CURDATE() + INTERVAL 1 DAY)) AND (e.state = 1) AND (e.approval <> 1) AND (e.catid=$selcat) AND ((e.language = '$langcurrent') OR (e.language = '*') OR (e.language = NULL) OR (e.language = ''))" ;
		}
		...
		if ($ctrlBack!=NULL){
			if ($getpage && $getpage<$pages) {
				$pageBack=$getpage-1;
				$pageNext=$getpage+1;
				$nav.='<a class="icagenda_back" href="'.$urlback.'page='.$pageBack.'">&#9668; '.$textback.'&nbsp;</a>';
				$nav.='<a class="icagenda_next" href="'.$urlnext.'page='.$pageNext.'">&nbsp;'.$textnext.' &#9658;</a>';
			}
			else {
				$pageBack=$getpage-1;
				$nav.='<a class="icagenda_back" href="'.$urlback.'page='.$pageBack.'">&#9668; '.$textback.'&nbsp;</a>';
	...
	protected function ManagerIcons ($i){
		...
		if($approved == 1) {
				$unapproved = '<small>';
			if(version_compare(JVERSION, '3.0', 'lt')) {
				$unapproved.= '<a class="ictip" href="'.$urlcheck.'" target="_blanck">&times;</i>';
			} else {
				$unapproved.= '<a class="ictip" href="'.$urlcheck.'" target="_blanck"><i class="icon-out-2"></i>';
			}
				$unapproved.= '<span>';
				$unapproved.= '<span class="title">';
				$unapproved.= ''.JText::_( 'COM_ICAGENDA_APPROVE_AN_EVENT_LBL' ).'';
				$unapproved.= '</span>';
				$unapproved.= '<div class="text">';
				$unapproved.= ''.JText::_( 'COM_ICAGENDA_APPROVE_AN_EVENT_DESC' ).'';
				$unapproved.= '</div>';
				$unapproved.= '</span>';
				$unapproved.= '</a>';
//				$unapproved.= '<a href="'.$urlcheck.'" target="_blanck" class="hasTooltip" title="'.JText::_( 'COM_ICAGENDA_UNAPPROVED' ).'"> <i class="icon-flag"></i></a>';
				$unapproved.= '</small>';
			$approval = $unapproved;
			return $approval;
		} else {
	...
	protected function image ($i){return $i->image;}
...
//				}
	...
	protected function fileTag ($i){
		return '<a class="icDownload" href="'.$i->file.'">'.JText::_( 'COM_ICAGENDA_EVENT_DOWNLOAD' ).'</a>';
	}
