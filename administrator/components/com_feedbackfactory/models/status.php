...
class BackendModelStatus extends JModelLegacy
{
  ...
  function &getData()
  {
    if (empty($this->_data))
    {
      $query = ' SELECT * FROM #__feedback_statuses'
             . ' WHERE id = '.$this->_id;
  ...
  function change()
  {
    ...
    $query = ' UPDATE #__feedback_statuses'
           . ' SET ' . $field . ' = ' . $value
           . ' WHERE id = ' . $id;
    $this->_db->setQuery($query);
  ...
  function delete()
  {
    ...
    $default_statuses = feedbackHelper::getDefaultStatuses();
    
    foreach($cids as $cid)
    {
      if (!in_array($cid,$default_statuses)) {
    	
        if (!$status->delete($cid))
        {
          $this->setError($this->_db->getErrorMsg());
          return false;
        }
